    ...
    try {
      ...
      if (schedules.length > 0) {
        ...
        tableBody.innerHTML = schedules.map(schedule => `
          <tr>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">${schedule.day || 'N/A'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${schedule.time || 'N/A'}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${schedule.course?.name || 'N/A'} (${schedule.course?.code || 'N/A'})</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${schedule.course?.lecturer?.name || 'N/A'}</td>
        ...
        cardContainer.innerHTML = schedules.map(schedule => {
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden border-l-4 ${colorClass}">
                <div class="p-4">
                    <p class="font-bold text-gray-800 text-lg truncate">${schedule.course?.name || 'Nama Mata Kuliah Tidak Tersedia'}</p>
                    <p class="text-sm text-gray-500 mb-4">(${schedule.course?.code || 'N/A'})</p>
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex items-center text-gray-700">
                            ...
                            <span class="font-semibold">${schedule.day || 'N/A'}</span>, <span>${schedule.time || 'N/A'}</span>
                        </div>
                        <div class="flex items-center text-gray-700">
                            ...
                            <span>${schedule.course?.lecturer?.name || 'Dosen Tidak Ditugaskan'}</span>
                        </div>
                        <div class="flex items-center text-gray-700">
                            ...
                            <span>Ruangan: ${schedule.room || 'N/A'}</span>
                        </div>
                    </div>
                </div>
            </div>
